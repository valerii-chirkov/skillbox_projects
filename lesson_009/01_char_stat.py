...
class StatMaker:
    ...
    def sort_chars(self):
        # todo Одна из важных частей изучаемого паттерна - новая функциолнальность добавляется не универсиализацией
    ...
    def sort_chars_alphabet_increase(self):  # 3) сортировка статистических данных (возрастание по алфавиту)
        ...
        for key in self.stat_keys:
            self.sorted_stat.append([key, self.stat[key]])  #  Он не совсем правильно сортирует, например буква ё в
            #  конце, может быть потому что она идет как спецсимвол у букв?
            # todo Символы Ёё находятся в стороне от остальных символов алфавита: 
            #  >> ord('ё')
            # Out[2]: 1105
            # >> ord('е')
            # Out[3]: 1077
            ...
            # >> ord('Ё')
            # Out[6]: 1025
            # todo Для данной задачи можно на этом не заострять внимание
    ...
    def sort_chars_alphabet_decrease(self):  # 3) сортировка статистических данных (убывание по алфавиту)
        self.stat_keys = list(self.stat.keys())
        self.stat_keys.sort()
        self.stat_keys.reverse()  # Есть ли в python'е что-то на подобие наследования от методов? Чтобы код не повторять
                                  # todo можно только вызвать метод предка, и что-то добавить к этому нового, вот так:
                                  #  def some_method()
                                  #      super().some_method()
                                  #      do some work
                                  #      return something
