...
class Context:
    ...
    def request2(self):
        self._state.handle2()
    # TODO главное что тут надо реализовать - общую проверка валидности (числа фреймов, запрещённые символы и т.п.) и
    #  обработку результата игры для подсчёта очков
...
class State(ABC):
    ...
    def handle2(self) -> None:
        pass
    # TODO тут удобно реализовать методы для: а) обработки текущего символа (собственно полная логика обработки);
    #   б) пара "абстрактных" методов которые будут реализованы в классах-наследниках - метод обработки страйка и метод
    #   обработки spare (в одном классе они будут именно считать, а в другом это будут выбрасывания исключений)
